...
public class SupplierRepository : ISupplierRepository
{
    ...
    public async Task<(IEnumerable<Supplier> Items, int TotalCount)> GetAllAsync(int pageSize = 10, int pageNumber = 1,
        string? searchTerm = null, bool? isActive = null, string? sortBy = "Name", bool sortDescending = false,
    ...
    public async Task<bool> NameExistsAsync(string name, Guid? excludeSupplierId = null,
        CancellationToken cancellationToken = default)
    {
        ...
        var result =
            await query.AnyAsync(
                s => s.Name.ToLower() == normalizedName &&
                     (excludeSupplierId == null || s.Id != excludeSupplierId.Value), cancellationToken);
