...
namespace InventoryManagement.Infrastructure.Repositories
{
    public class StockTransactionRepository : IStockTransactionRepository
    {
        ...
        public async Task<StockTransaction> AddAsync(StockTransaction stockTransaction, CancellationToken cancellationToken = default)
        {
        ...
        public async Task<(IEnumerable<StockTransaction>, int totalCount)> GetAllAsync(int pageNumber = 1, int pageSize = 10, string? keyword = null, CancellationToken cancellationToken = default)
        {
            ...
            else
            {
                stockTransactions = allTransactions
                    ...
                    (st?.Supplier?.ContactPerson != null && st.Supplier.ContactPerson.Contains(keyword, StringComparison.OrdinalIgnoreCase) ||
                    (st?.ProductBatch?.BatchNumber != null && st.ProductBatch.BatchNumber.Contains(keyword, StringComparison.OrdinalIgnoreCase))
                    ));
        ...
        public async Task<IEnumerable<StockTransaction>> GetByProductIdAsync(Guid productId, CancellationToken cancellationToken = default)
        {
            return await _context.StockTransactions
